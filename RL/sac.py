...
def main():
    ...
    parser.add_argument(
        "--total_timesteps", type=int, default=1000000, help="Number of timesteps"
    )
    parser.add_argument(
        "--num_envs", type=int, default=4, help="Number of envs to run in parallel"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Hidden size for MLP (used twice: [h, h])",
    )
    ...
    actor = SoftActorNet(obs_dim, action_dim, (act_low, act_high), hidden_sizes).to(
        device
    )
    ...
    q_optim = torch.optim.Adam(
        list(q1c.parameters()) + list(q2c.parameters()), lr=args.q_lr
    )
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            s_obs, s_next_obs, s_actions, s_rewards, s_dones = buffer.sample(
            ...
            with torch.no_grad():
                ...
                q_target = s_rewards + args.gamma * (1 - s_dones) * (
                    torch.min(q1t, q2t) - alpha * next_state_log_probs
                )
            ...
            if global_step % args.target_network_frequency == 0:
                for param, target_param in zip(
                    q1c.parameters(), q1c_target.parameters()
                ):
                    target_param.data.copy_(
                        args.tau * param.data + (1 - args.tau) * target_param.data
                    )
                for param, target_param in zip(
                    q2c.parameters(), q2c_target.parameters()
                ):
                    target_param.data.copy_(
                        args.tau * param.data + (1 - args.tau) * target_param.data
                    )
