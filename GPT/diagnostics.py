...
class TrainingDiagnostics:
    ...
    def log_activation_stats(
        ...
        if self.activation_stats:
            avg_dead = sum(s['dead_frac'] for s in self.activation_stats.values()) / len(self.activation_stats)
    ...
    def get_summary(self, lr: float) -> Dict:
        ...
        # Find extreme layers
        max_grad_layer = max(grad_stats.items(), key=lambda x: x[1].grad_norm) if grad_stats else (None, None)
        min_grad_layer = min(grad_stats.items(), key=lambda x: x[1].grad_norm) if grad_stats else (None, None)
        
        total_weight_norm = math.sqrt(sum(s.weight_norm ** 2 for s in weight_stats.values()))
        avg_update_ratio = sum(s.update_ratio for s in weight_stats.values()) / len(weight_stats) if weight_stats else 0
        
        return {
            'total_grad_norm': total_grad_norm,
            'total_weight_norm': total_weight_norm,
            'avg_update_ratio': avg_update_ratio,
            'max_grad_norm_layer': (max_grad_layer[0], max_grad_layer[1].grad_norm) if max_grad_layer[0] else None,
            'min_grad_norm_layer': (min_grad_layer[0], min_grad_layer[1].grad_norm) if min_grad_layer[0] else None,
        }
...
def detect_anomalies(
    ...
    if prev_loss is not None and loss > prev_loss * loss_spike_threshold:
        warnings.append(f"WARNING: Loss spiked from {prev_loss:.4f} to {loss:.4f} ({loss/prev_loss:.1f}x)")
