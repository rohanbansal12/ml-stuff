...
import torch
import torchvision
from diffusion import DDIMSampler, DDPMSampler, NoiseSchedule
...
def main():
    ...
    if args.sampler == "ddpm":
        ...
        sampler = DDIMSampler(
            schedule, config.diffusion.pred_type, num_inference_steps=args.steps, eta=args.eta
        )
    ...
    while num_generated < args.num_samples:
        ...
        with torch.no_grad():
            samples = sampler.sample(model, (batch_size, 3, 32, 32), device, progress=True)
