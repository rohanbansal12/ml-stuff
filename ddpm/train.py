...
from diffusion import NoiseSchedule, DDPMSampler, q_sample, get_target
...
def main():
    ...
    print(f"Diffusion: T={config.diffusion.T}, pred={config.diffusion.pred_type}, schedule={config.diffusion.schedule_type}")
    ...
    for epoch in range(start_epoch, config.train.epochs):
        train_loss = train_one_epoch(
            model, train_loader, optimizer, schedule, config,
            device, ema, writer, epoch, scaler, autocast_dtype
        )
        ...
        if (epoch + 1) % config.train.sample_every == 0:
            ...
            samples = generate_samples(
                ...
                config.train.num_samples, fixed_noise
            )
        ...
        if (epoch + 1) % config.train.save_every == 0:
            save_checkpoint(
                ...
                model, optimizer, ema, epoch, config, train_loss
            )
    ...
    save_checkpoint(
        ...
        model, optimizer, ema, config.train.epochs - 1, config, train_loss
    )
