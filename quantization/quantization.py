...
class QuantConfig:
    ...
    group_size: int | None = None  # Group quantization (e.g., 128 for GPTQ)

    # Layer filtering: functions that take layer name and return bool
    # Layer is quantized if: include(name) and not exclude(name)
    include: LayerFilter | None = None  # If None, include all
    exclude: LayerFilter | None = None  # If None, exclude none
...
def quantize_rtn(
    ...
    """
    if config.symmetric:
        scale = compute_scale_symmetric(tensor, bits=config.bits, per_channel=config.per_channel)
    ...
    dq = dequantize_tensor(q, scale=scale, zero_point=zero_point, symmetric=config.symmetric)
    ...
    if sequential:
        ...
        for layer_name, layer in tqdm(linear_layers, desc="GPTQ (sequential)"):
            activations = collect_calibration_activations(
                new_model, calibration_data=calibration_data, layer_name=layer_name
            )

            hessian = compute_hessian(activations)

            dq_weight, state = quantize_gptq_layer(layer.weight, hessian=hessian, config=config)
        ...
        all_activations = collect_all_layer_activations(
            new_model, calibration_data, layer_names, store_on_cpu=True
        )
...
def quantize_awq_layer(
    ...
    scales = find_optimal_scales(weight, activation_magnitudes=activation_magnitudes, config=config)
    ...
    if sequential:
        ...
        for layer_name, layer in tqdm(linear_layers, desc="AWQ (sequential)"):
            ...
            quantized_weight, scales, state = quantize_awq_layer(
                layer.weight, activation_magnitudes=activation_magnitudes, config=config
            )
...
def test_scale_symmetric():
    ...
    assert torch.allclose(s, torch.tensor(expected), atol=1e-6), f"Expected {expected}, got {s}"
    ...
    assert torch.allclose(
        s.flatten()[0], torch.tensor(expected_0), atol=1e-6
    ), f"Channel 0: expected {expected_0}, got {s.flatten()[0]}"
    assert torch.allclose(
        s.flatten()[1], torch.tensor(expected_1), atol=1e-6
...
def test_rtn():
    ...
    assert error_pc["snr_db"] >= error["snr_db"] - 0.1, "Per-channel should be >= per-tensor"
...
def test_layer_filtering():
    ...
    config = QuantConfig(
        bits=8, symmetric=True, per_channel=True, exclude=lambda n: "embed" in n or "lm_head" in n
    )
    model_q = quantize_model_rtn(model, config)
    ...
    assert torch.allclose(
        model.lm_head.weight, model_q.lm_head.weight
    ...
    config = QuantConfig(bits=8, symmetric=True, per_channel=True, include=lambda n: "mlp" in n)
    model_q = quantize_model_rtn(model, config)
