...
from quantize import quantize_symmetric, calc_qparams_symmetric
...
class QuantLinearW8A8StaticAct(nn.Module):
    def __init__(self, in_features: int, out_features: int, has_bias: bool):
        ...
        self.register_buffer("last_x", torch.empty((in_features, out_features), dtype=torch.float32), persistent=False)

        self.register_buffer("w_q", torch.empty((out_features, in_features), dtype=torch.int8), persistent=True)
        self.register_buffer("s_w", torch.empty((out_features, 1), dtype=torch.float32), persistent=True)
    ...
    @classmethod
    def from_float(cls, layer: nn.Linear, s_x: float, zp_x: int):
    ...
    # def forward(self, x: torch.Tensor):
    #     # x: [B, T, in_features] for OPT internals (or [*, in_features])
...
def quantize_model_w8a8(model, act_stats):
    ...
    for name, mod in to_replace:
        ...
        new_mod = QuantLinearW8A8StaticAct.from_float(mod, s_x=s_x, zp_x=zp_x)
...
def main():
    ...
    # Evaluate FP32 GEMM
    fp32g_model = force_fp32_gemm_all_linears(copy.deepcopy(fp_model), include_filter=include, exclude_filter=exclude)
    ...
    q_model = quantize_model_w8a8(q_model, act_stats)
