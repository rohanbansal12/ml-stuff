...
def make_filter(
    patterns: Optional[list[str]], match_any: bool = True
) -> Optional[LayerFilter]:
    """
...
def move_calibration_to_device(
    calibration_data: list[torch.Tensor], device
) -> list[torch.Tensor]:
...
def run_comparison(
    ...
    print(
        f"  Params quantized: {mem_stats['quantized_params']:,} / {mem_stats['total_params']:,}"
    )
    ...
    if run_calibration_methods:
        ...
        try:
            ...
            model_gptq = quantize_model_gptq(
                model, calibration_data, config_gptq, sequential=False
            )
        ...
        try:
            # Use fast mode (sequential=False) by default
            model_awq = quantize_model_awq(
                model, calibration_data, config_awq, sequential=False
            )
    ...
    if run_calibration_methods:
        print(
            f"  Calibration: {calibration_samples} samples × {calibration_seq_len} tokens"
        )
    ...
    for name, exp in all_results["experiments"].items():
        ...
        print(
            f"{exp['precision']:<30} {acc:>12.4f} {ppl:>10.2f} {delta_acc:>+10.4f} {delta_ppl:>+10.2f}"
        )
...
Examples:
  ...
  # Full comparison with calibration methods and more samples
  python compare_quant.py --bits 4 --calibration --calibration-samples 256
...
Exclude presets: none, default, embed_only, head_only
    ...
    parser.add_argument(
        "--bits", type=int, default=8, choices=[8, 4, 2], help="Quantization bit width"
    )
    parser.add_argument(
        "--include",
        type=str,
    ...
    parser.add_argument(
        "--wikitext-tokens", type=int, default=4096, help="Number of WikiText tokens"
    )
    parser.add_argument(
        "--calibration",
        action="store_true",
